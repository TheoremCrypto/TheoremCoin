...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("theoremcoin"))
        return false;
...
{
    // Convert theoremcoin:// to theoremcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("theoremcoin://"))
    {
        uri.replace(0, 12, "theoremcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "TheoremCoin.lnk";
}
...
{
    return GetAutostartDir() / "theoremcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("TheoremCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  theoremcoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("TheoremCoin-Qt"));
